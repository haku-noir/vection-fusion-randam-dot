...
def create_occupancy_bar_chart(occupancy_data_list, condition, output_dir):
    ...
    try:
        ...
        colors = ['lightcoral', 'lightgreen', 'orange', 'lightblue']  # 赤のみ、緑のみ、両方高、両方低
        labels = ['赤ドットのみ≥0.5', '緑ドットのみ≥0.5', '両方≥0.5', '両方<0.5']
        
        for i, (subject, sessions) in enumerate(subjects.items()):
            ax = axes[i]
            ...
            bars3 = ax.bar(x_positions, both_high, bottom=np.array(red_only) + np.array(green_only), color=colors[2], label=labels[2])
            bars4 = ax.bar(x_positions, both_low, bottom=np.array(red_only) + np.array(green_only) + np.array(both_high), color=colors[3], label=labels[3])
            
            # 各セクションに％数値を表示
            for j, (pos, r_only, g_only, b_high, b_low) in enumerate(zip(x_positions, red_only, green_only, both_high, both_low)):
                ...
                if b_high >= 5.0:
                    ax.text(pos, r_only + g_only + b_high/2, f'{b_high:.1f}%', ha='center', va='center', 
                           fontsize=8, fontweight='bold', color='white')
                
                # 両方<0.5の数値表示（5%以上の場合のみ表示）
                if b_low >= 5.0:
                    ax.text(pos, r_only + g_only + b_high + b_low/2, f'{b_low:.1f}%', ha='center', va='center', 
                           fontsize=8, fontweight='bold', color='black')
            
            # セッションIDをx軸ラベルに設定
            session_labels = [s['session_id'] for s in sessions]
            ax.set_xticks(x_positions)
            ax.set_xticklabels(session_labels, rotation=45, ha='right')
            
            ax.set_title(f'{subject}', fontsize=14)
            ax.set_ylim(0, 100)
